...
A central contribution of this work is the observation that all 11 recursive digital filter methods implemented in \texttt{baseflowx} can be expressed as special cases of a single three-parameter equation:
\begin{equation}
...
Recession analysis underpins many baseflow separation approaches, as the recession curve encodes information about aquifer storage and drainage characteristics~\cite{Tallaksen1995,Wittenberg1999}. The BFlow method~\cite{Arnold1999} estimates the SWAT model's baseflow recession constant (ALPHA\_BF) by fitting the exponential recession model:
\begin{equation}
\label{eq:bflow}
...
The source code is hosted at \url{https://github.com/BYU-Hydroinformatics/baseflowx} under the MIT license. Documentation built with MkDocs is available at \url{https://baseflowx.readthedocs.io/en/latest/}, including method guides with equations and figures, practical tutorials, and auto-generated API reference.
...
The results above, combined with the theoretical framework in Section~\ref{sec:framework}, suggest the following practical guidance for method selection:

\begin{itemize}[nosep]
\item When recession coefficients can be reliably estimated and physical interpretability is desired, the \textbf{Eckhardt filter} is recommended due to its $\mathrm{BFI_{max}}$ parameterization and well-documented guidelines~\cite{Eckhardt2005,Collischonn2013}.
\item When catchment parameters are uncertain, \textbf{graphical methods} (HYSEP, UKIH) require only drainage area or no parameters at all, making them robust defaults.
\item When water-quality data are available, the \textbf{CMB method} provides an independent, physically grounded estimate and can serve as a calibration target for digital filters via the CMB-to-Eckhardt bridge.
\item For consistency with USGS studies, the \textbf{PART method} is directly comparable to legacy Fortran outputs.
\item When multiple methods are applied, the \textbf{ensemble spread} provides a practical uncertainty estimate, as the true baseflow likely falls within the range of method-specific estimates~\cite{Eckhardt2008}.
\end{itemize}
...
Table~\ref{tab:comparison} compares \texttt{baseflowx} with existing baseflow separation tools across several dimensions relevant to modern hydrological practice.

\begin{table}[H]
\caption{Comparison of \texttt{baseflowx} with existing baseflow separation tools.\label{tab:comparison}}
\small
\begin{tabularx}{\textwidth}{lcccc}
\toprule
\textbf{Feature} & \textbf{baseflowx} & \textbf{HYSEP/PART} & \textbf{WHAT} & \textbf{baseflow}~\cite{Xie2020} \\
\midrule
...
The USGS programs HYSEP and PART~\cite{Sloto1996,Rutledge1998} remain widely used in regulatory and research contexts, but their Fortran executables cannot be called from Python scripts, Jupyter notebooks, or cloud-based workflows without cumbersome wrappers. The WHAT tool~\cite{Lim2005} offers web accessibility but is limited to three methods and cannot be automated for batch processing. The original \texttt{baseflow} package by Xie et al.~\cite{Xie2020} provided a Python implementation of 11 digital filters but included study-specific geospatial functionality (global freeze/thaw rasters, spatial batch processing) that complicated its use as a general-purpose library, and it lacked graphical, recession-based, and tracer-based methods. R-based alternatives such as the parametric approach of Pelletier and Andrassian~\cite{Pelletier2020} offer single-method implementations but do not provide the breadth of methods needed for cross-method comparison studies.

By contrast, \texttt{baseflowx} combines the broadest method coverage (17 methods across four paradigms) with a clean, focused API and the unified filter framework described in Section~\ref{sec:generalized_filter}. Its integration into the Python scientific ecosystem---alongside NumPy~\cite{Harris2020}, pandas~\cite{McKinney2010}, and complementary hydrology packages such as Hydrostats~\cite{Roberts2018} and Pastas~\cite{Collenteur2019}---enables seamless incorporation into reproducible analytical workflows.
...
We have presented \texttt{baseflowx}, an open-source Python package that unifies 17 baseflow separation methods under a consistent API. The package spans four methodological paradigms---recursive digital filters, graphical methods, recession analysis, and tracer-based approaches---making it the most comprehensive Python tool available for baseflow separation.
...
The package is designed for ease of use: it is pip-installable, provides built-in USGS data retrieval, includes bundled sample data, and is accompanied by comprehensive documentation with method guides and worked examples. We hope that \texttt{baseflowx} will serve as a practical tool for hydrologists, water resource engineers, and researchers, and that its open-source nature will foster community contributions and continued development.
...
Pelletier, A.; Andrassian, V. Hydrograph Separation: An Impartial Parametrisation for an Imperfect Method. \textit{Hydrol. Earth Syst. Sci.} \textbf{2020}, \textit{24}, 1171--1187. \url{https://doi.org/10.5194/hess-24-1171-2020}
